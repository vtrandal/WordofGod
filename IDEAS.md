...
* **Details**:
  ...
  * **Synchronized Continuous Narration (Word-of-Promise Experience)**: Integrated continuous chapter narration engine with real-time synchronized verse highlighting and automatic scrolling (demonstrated on Genesis 1). Uses a single continuous text utterance with `onboundary` character-offset tracking to completely bypass iOS Safari's 2-utterance user-gesture timeout and queue deadlocks.
